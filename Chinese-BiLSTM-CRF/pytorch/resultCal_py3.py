...
def calculate(x,y,id2word,id2tag,res=[]):
    ...
    for j in range(len(x)):
        ...
        elif id2tag[y[j]][0]=='M' and len(entity)!=0 and entity[-1].split('/')[1][1:]==id2tag[int(y[j])][1:]:
            entity.append(id2word[int(x[j])]+'/'+id2tag[int(y[j])])
        elif id2tag[y[j]][0]=='E' and len(entity)!=0 and entity[-1].split('/')[1][1:]==id2tag[int(y[j])][1:]:
            entity.append(id2word[int(x[j])]+'/'+id2tag[int(y[j])])
            entity.append(str(j))
            res.append(entity)
            entity=[]
